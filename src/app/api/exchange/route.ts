...
// ═══════════════════════════════════════════════════════════════════════════
// GET — Price preview & allocation estimate
// ═══════════════════════════════════════════════════════════════════════════
export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);
    const fromAsset = searchParams.get("fromAsset") || searchParams.get("fromToken");
    const toAsset = searchParams.get("toAsset") || searchParams.get("toToken");
    const amount = parseFloat(searchParams.get("amount") || "0");
    const type = searchParams.get("type") || "buy"; // for backwards compat
    ...
    // Server-side price calculation (same as POST)
    const { toAmount, fromPrice, toPrice, tradingFee, custodyFee, totalFeePercent } =
      await calculateServerToAmount(fromAsset, toAsset, amount);

    const fee = amount * fromPrice * (totalFeePercent / 100);

    // Allocation preview (metal purchases only)
    ...
    if (METALS.includes(toKey)) {
      ...
      if (nonAllocatedGrams > 0) {
        ...
        const additionalNeeded = parseFloat(
          (gramsNeededForNextWhole * toPrice / (1 - totalFeePercent / 100)).toFixed(2)
        );
    ...
    return NextResponse.json({
      ...
      preview: {
        ...
        price: parseFloat(toPrice.toFixed(4)),
        fee: parseFloat(fee.toFixed(4)),
        feePercent: totalFeePercent,
        fromPrice: parseFloat(fromPrice.toFixed(4)),
        toPrice: parseFloat(toPrice.toFixed(4)),
        tradingFee,
        custodyFee,
        allocationPreview,
      },
    ...
    return NextResponse.json({
      ...
      exchange: {
        ...
        fees: {
          ...
          custodyFee,
          totalFeePercent,
          feeUsd: parseFloat((fromAmount * fromPrice * totalFeePercent / 100).toFixed(4)),
        },
