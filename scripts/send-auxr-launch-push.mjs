// scripts/send-auxr-launch-push.mjs
// Broadcast push for the "AUXR is live" launch, with per-user language
// lookup (Redis user:{id}.language or auth:user:{email}.language, fallback
// 'en'). Targets every wallet in `push:mobile:all_users`.
//
// DRY-RUN BY DEFAULT. Pass --send to actually deliver.
//
// Recent-send guard: each delivered wallet is recorded as
// `push:auxrlaunch:sent:{wallet}` (TTL 30 days) — safe to re-run.
//
// Usage:
//   node scripts/send-auxr-launch-push.mjs                  # dry-run, all
//   node scripts/send-auxr-launch-push.mjs --limit=5        # dry-run sample
//   node scripts/send-auxr-launch-push.mjs --limit=5 --send # send 5
//   node scripts/send-auxr-launch-push.mjs --send           # send to everyone
//   node scripts/send-auxr-launch-push.mjs --test=0x.. --send # one wallet

import { Redis } from "@upstash/redis";
import { config } from "dotenv";
import { fileURLToPath } from "url";
import { dirname, resolve } from "path";
import { getPushContent } from "./auxr-launch-content.mjs";
...
async function alreadySent(walletAddress) {
  const v = await redis.get(`push:auxrlaunch:sent:${walletAddress.toLowerCase()}`);
  return !!v;
}

async function markSent(walletAddress) {
  await redis.set(`push:auxrlaunch:sent:${walletAddress.toLowerCase()}`, Date.now(), {
...
async function main() {
  console.log(`\n${"━".repeat(72)}`);
  console.log(`🧺 AUXR Launch Push — ${DRY_RUN ? "DRY-RUN" : "LIVE SEND"}`);
